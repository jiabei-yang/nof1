...
summarize_nof1 <- function(nof1, result, treat.name){
...
wrap <- function(data, metadata){
  ...
  stool_frequency <- tryCatch({
    ...
    # nof1 <- nof1_freq
    # result <- result_freq
    summarize_nof1(nof1_freq, result_freq, treat.name)
  }, error = function(error){
  ...
  stool_consistency <- tryCatch({
    ...
    # nof1 <- nof1_cons
    # result <- result_cons
    summarize_nof1(nof1_cons, result_cons, treat.name)
  }, error = function(error){
  ...
  pain_interference <- tryCatch({
    ...
    # nof1 <- nof1_pain
    # result <- result_pain
    summarize_nof1(nof1_pain, result_pain, treat.name)
  }, error = function(error){
  ...
  gi_symptoms <- tryCatch({
    ...
    # nof1 <- nof1_gi
    # result <- result_gi
    summarize_nof1(nof1_gi, result_gi, treat.name)
  }, error = function(error){
